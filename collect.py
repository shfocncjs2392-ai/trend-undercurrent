...
GROQ_API_KEY = "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"
try:
...
# --- [1] 설정 및 초기화 ---
NAVER_CLIENT_ID = "mDUgUgcdMfEj8ZI2bZ9v"
NAVER_CLIENT_SECRET = "KROEccn3Dx"
YOUTUBE_API_KEY = "AIzaSyAFDYze-4VgJICCTMcpnCKliHS63cQERbg"
...
# --- [RDS 연결 정보 설정] ---
DB_HOST = "trend-undercurrent-db.ctysyyia4d93.eu-west-2.rds.amazonaws.com"
DB_USER = "admin"
DB_PASS = "Whdydfla2392"  # 아까 RDS 만들 때 넣은 암호!
DB_NAME = "trend_db"
...
def get_ai_insight(kw, related, total_search, search_dod, yt_views, yt_videos, trend_type):
    ...
    prompt = f"""너는 트렌드 분석가야. 아래 데이터를 보고 '{kw}'가 왜 현재 트렌드인지 원인을 분석해줘.
    [데이터 정보]
    - {search_info}
    - {yt_info}
    - 연관 키워드: {related}
    ...
    분석 가이드:
    1. 데이터 수치(증감률 등)를 언급하며 신뢰도를 높여.
    2. '왜' 이 현상이 일어났는지 뉴스나 SNS 트렌드 문맥을 섞어서 3줄 이내로 요약해.
    3. 현재 Trend Type에 대한 이유를 설명해야해. (예를 들어 관측중이라면 관측중인 이유, 감소중이라면 감소중인 이유)
    3. 구체적인 브랜드나 사건이 있다면 반드시 언급해."""
